...
const workCardContainer = document.getElementsByClassName(
  "work-card-container",
)[0];
...
function displayWorkExpCard() {
  for (let i = 0; i < workInfo.length; i++) {
    let workCompany = createElement("div", "work-company");
    let workCard = createElement("div", "work-card");
    let logoTitleHolder = createElement("div", "logo-title");
    
    // company logo
    let logo = createImg(
        workInfo[i].logo,
        `${workInfo[i].company} logo`,
        "company-logo",
    );
    ...
    downArr = createImg(
      "./img/down-arrow-1.png",
      "Down Arrow",
      "toggle-button",
    );
    imgContainer.appendChild(downArr);

    logoTitleHolder.appendChild(logo);
    logoTitleHolder.appendChild(companyName);
    workCompany.appendChild(logoTitleHolder);
    ...
    companyDetailsDesc = createElementAndAppend(
      "p",
      "",
      workInfo[i].companyDetails,
    );
...
function toggleWorkCard() {
  ...
  for (let i = 0; i < workDetails.length; i++) {
    let isShown = false;
    const toggleButton =
      workDetails[i].previousSibling.childNodes[1].firstChild;
